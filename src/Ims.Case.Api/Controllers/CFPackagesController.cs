...
namespace Ims.Case.Controllers
{
    ...
    public partial class CFPackagesController : ControllerBase
    {
        private readonly ICaseApiSupervisor _supervisor;

        public CFPackagesController(ICaseApiSupervisor supervisor)
        {
        ...
        [SwaggerOperation(
            ...
            Tags = new string[]{"PackagesManager"},
        ...
        public async Task<IActionResult> GetCFPackageAsync(
            [SwaggerParameter(
            ...
            CancellationToken ct = default)
        {
            try
            {
                var retVal = await _supervisor.GetCFPackageByIdAsync(sourcedId, ct);
                if (retVal == null)
                {
                    return StatusCode(StatusCodes.Status400BadRequest, ErrorFactory.InvalidUUID);
                }
                return StatusCode(StatusCodes.Status200OK, retVal);
            }
            catch (Exception e)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, ErrorFactory.ServerError);
            }
        }
    }
}
